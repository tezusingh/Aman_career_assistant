# career-ops-ui — full-project regression prompt (v1.26.0)
...
> v1.6.0 baseline through v1.26.0. Catches regressions in code that
> already works; **does not** describe known open backlog (see
> `qa/FIX-PROMPT-v24.1.md`).
> **Owner:** this file is the canonical regression source. Older prompts
> in `qa/archive/` are historical only. The `qa/v24-regression/` folder
> holds evidence from prior runs.
...
# expected: ok=true, version=1.26.0 (or newer), all `required: true` checks ok
...
**PASS** = 13 sidebar + 8 mode + 2 alias routes render with 0 console errors.
...
Each block can be wrapped into a test script. Output → `qa/v26-regression/<scenario>.log`.
...
### 6.6 CHANGELOG locale parity (v1.25.0 + v1.26.0 gate)

```bash
node scripts/check-changelog-parity.mjs
# Expected: "✓ CHANGELOG parity: all 8 locales at v1.26.0"
```
...
Generate a single Markdown file under `qa/v26-regression/<run-date>-REGRESSION.md`:

```markdown
# Regression run — YYYY-MM-DD

**Stand:** http://127.0.0.1:4317 · v1.26.0 · parentVersion 1.7.1 · Node 22.x
...
|---|---|---|---|---|---|
| 1. Smoke nav | 23 routes | 23 | 0 | 0 | |
| 2.1 Dashboard | 5 | 5 | 0 | 0 | |
| 2.2 Scan | 4 | 4 | 0 | 0 | |
| …  | | | | | |
| 8. Per-release | 50+ | 50 | 0 | 0 | |
| **Total** | **150+** | **N** | **M** | **K** | |
...
| **MEDIUM** | 2.1-2.6 individual feature regressions, 6.5 (help-bundle content depth), 7.1-7.5 (docs alignment) |
...
npm run test:ci                        # 502 unit + 2 CI gates
