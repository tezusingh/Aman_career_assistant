...
> **Built:** 2026-05-14, against `web-ui@1.29.0`.
...
```bash
curl -fsS http://127.0.0.1:4317/api/health | jq '.version'   # → "1.29.0" (or newer)
...
| 18 | Russian portals (`russian_portals.sources`, `area`, `per_page`, `only_remote`, `queries`) | **EXTRA** (web-ui addition beyond canonical docs — v1.29.0 expanded from 2 → 5 sources) | `#/scan` SSE runs **5 RU sources** by default (hh.ru / Habr Career / Trudvsem / GetMatch / GeekJob); source-filter dropdown is dynamic via `GET /api/scan/sources` → `registry.mjs` | `GET /api/scan/regional/config` + `GET /api/scan/sources` | `tests/scan-regional.test.mjs` + `sources-trudvsem.test.mjs` + `sources-getmatch-geekjob.test.mjs` + `scan-sources-endpoint.test.mjs` + `ru-scanner.test.mjs` | help-bundle §17 "How to add a portal" × 8 locales |
...
| Filters: Free text / **Source dropdown** (v1.29.0 — built dynamically from `GET /api/scan/sources`, must show 11 entries: Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday / GeekJob / GetMatch / Habr Career / hh.ru / Trudvsem) / Remote-Hybrid-Onsite / Stack chips (PHP / Go / Backend / Senior / …) / Dynamic chips | every dropdown + chip carries i18n key × all 8; source dropdown populated from server response (fallback list at build time mirrors registry) |
...
### 3.F — Russian portals (`hh.ru` + Habr Career + Trudvsem + GetMatch + GeekJob) — v1.29.0

- **Docs:** silent on regional portals (career-ops.org/docs covers only English ATSes).
- **Our SPA:** `#/scan` integrates **5 RU sources** by default (was 2 pre-v1.29.0):
  - `hh.ru` — public API at `api.hh.ru`. Geo-aware UA fallback (`HH_USER_AGENT` env var). Non-RU IPs may 403.
  - `Habr Career` — HTML scrape of `career.habr.com`. Works from any IP.
  - `Trudvsem` — Russian government open-data API at `opendata.trudvsem.ru/api/v1/vacancies`. No auth, no IP gate.
  - `GetMatch` — tech-focused HTML scrape (`getmatch.ru`). Defensive parser, `[]` on parse miss.
  - `GeekJob` — IT-focused HTML scrape (`geekjob.ru`). Same defensive style as GetMatch.
- **Registry as single source of truth:** [`server/lib/sources/registry.mjs`](../server/lib/sources/registry.mjs) lists all 11 adapters (6 EN ATS + 5 RU). The dispatcher in [`ru-scanner.mjs::RU_DISPATCH`](../server/lib/ru-scanner.mjs), the API endpoint `GET /api/scan/sources`, and the SPA's `#/scan` dropdown all consume it. Adding a 12th source = one entry here + one adapter file + one row in `RU_DISPATCH`.
- **Help-bundle §17 "How to add a new job-portal source"** × 8 locales documents the full developer flow (adapter template, registry entry, dispatcher wiring, mocked unit test, `portals.yml` enablement).
- **`russian_portals.sources` default** changed from `["hh", "habr"]` to all 5 registry entries (`RU_CONFIG_KEYS`). Pre-v1.29 stands with `sources:` already listed in `portals.yml` must add the 3 new keys manually — web-ui never edits parent-project files.
- **Status:** **EXTRA + DOCUMENTED** — web-ui addition beyond canonical docs. QA must verify all 5 fire on a fresh stand (no `sources:` override in `portals.yml`).
